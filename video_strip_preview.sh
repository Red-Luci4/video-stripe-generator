...
INPUT="$1"
OUTPUT="Preview_$(echo "$(basename "$1")"|sed -E "s~(.*\.)[^\.]*$~\1jpg~g")"
echo "output = $OUTPUT"
TITLE="$(basename "$1")"
echo "title = $TITLE"
TITLE_SAN=$(echo $TITLE|sed -E 's~[^a-z^A-Z^0-9^\.^ ^\-^_]~~g')
SIZE=360
BORDER=10
PADDING=10
COLUMN=5
...
TIME_FONT_SIZE=$(($(($(( W_Frame > H_Frame ? W_Frame : H_Frame ))/$(( W_Frame < H_Frame ? W_Frame : H_Frame )) * W_Frame)) / 10))
